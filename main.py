    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "⛔ Remove item", switch_inline_query_current_chat="/done "
            )
        ],
...
def get_confirmation_keyboard():
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ Yes, delete everything", callback_data="confirm_clear"
            )
        ],
        [
            InlineKeyboardButton(
                "📝 No, please keep ...",
                switch_inline_query_current_chat="/clearexcept ",
            )
        ],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Hi, I am Shoppy 🤖 I'll help you shop.")
    ...
    try:
        if menu_message_id:
            ...
            message = await context.bot.send_message(
                chat_id=chat_id, text=message_text, reply_markup=get_main_keyboard()
            )
...
async def add_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not full_input_string:
        await msg.reply_text(
            "Please let me know what should be added. E.g.: /add 2x milk, 1L juice, bread"
        )
        return
...
async def done_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def handle_error(text):
        ...
        context.chat_data.setdefault("last_command_message_ids", []).append(
            msg.message_id
        )
        error_message = await msg.reply_text(text)
        context.chat_data.setdefault("last_error_message_ids", []).append(
            error_message.message_id
        )

    if not args_string:
        await handle_error(
            "Please type in the numbers of items which should be removed. E.g.: /done 1, 2 5"
        )
        return
    ...
    if not number_strings:
        await handle_error(
            "I could not find valid numbers in your message. Please try again."
        )
        return
    ...
    if invalid_numbers:
        await handle_error(
            f"The numbers {', '.join(invalid_numbers)} are invalid and will be ignored."
        )

    if ids_to_delete:
        # Clean up all previous temporary messages (errors and commands)
        messages_to_delete = context.chat_data.pop(
            "last_error_message_ids", []
...
async def clear_except(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args_string:
        await msg.reply_text(
            "Please type the numbers of items which should be kept. E.g.: /clearexcept 1,3"
        )
        return
...
def main():
    ...
    application.add_handler(
        MessageHandler(filters.Regex(r"/clearexcept"), clear_except)
    )
